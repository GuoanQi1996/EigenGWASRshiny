...
hist(fq$ALT_FREQS, main="MAF", xlab="Minor allele frequency", xlim=c(0, 0.5), breaks = 50)
```
...
Ne=params$ne
Me=params$me
hist(params$offDiag/params$sc, main="Pairwise relatedness", xlab="Relatedness score", breaks = 50)
        
nn=params$nn
mm=params$mm
legend("topright", legend = c(paste0("ne=", format(Ne, digits=3, nsmall=2), ' [',nn, ']'), paste0("me=", format(Me, digits=3, nsmall=2), ' [',mm,']')), bty='n')

hist(params$Diag/params$sc, main="Self-relatedness", xlab="Relatedness score", breaks = 15)
```
...
for(i in 1:PC) {
  ...
  if(!file.exists(EigenResName)){
    EigenRes=read.table(paste0(froot, ".PC", i, ".glm.linear"), as.is = T, header = F, 
                        colClasses = c("numeric","numeric","character","NULL","NULL","NULL","NULL","NULL","NULL","NULL","numeric","numeric","NULL"))
    colnames(EigenRes)=c("CHR","BP","SNP","STAT","P")
    ...
    manhattan(man_cache,significant = 0.05/nrow(EigenRes),cex = 0.4,colorSet = c("grey","darkblue"),title=paste0("ePC",i))
    dev.off()
    ...
    qqPlotQ(qq_cache_raw,ci.level = NULL, cex = 0.4,title=paste0("ePC",i))
...
[Guo-An Qi et al, EigenGWAS: An online visualizing and interactive application for detecting genomic signatures of natural selection, Molecular Ecology Resources, 2021, 21:1732-1744.](https://onlinelibrary.wiley.com/doi/10.1111/1755-0998.13370)
